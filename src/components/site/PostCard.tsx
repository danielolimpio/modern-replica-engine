...
export function PostCard({ post, variant = "default" }: { post: Post; variant?: "default" | "compact" | "list" | "featured" }) {
  if (variant === "compact") {
    return (
      <Link to="/post/$slug" params={{ slug: post.slug }} className="group flex min-w-0 flex-1 gap-3">
  ...
  if (variant === "list") {
    return (
      <article className="group grid grid-cols-[minmax(0,1fr)_auto] gap-4 border-b border-border/60 py-5 sm:grid-cols-[1fr_180px]">
        ...
        <Link to="/post/$slug" params={{ slug: post.slug }} className="relative aspect-[4/3] w-32 shrink-0 overflow-hidden rounded-md bg-secondary sm:w-[180px]">
          {post.thumb && <img src={post.thumb} alt="" loading="lazy" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />}
        </Link>
      </article>
    );
  ...
  if (variant === "featured") {
    return (
      <Link to="/post/$slug" params={{ slug: post.slug }} className="group relative block h-full min-h-[380px] overflow-hidden rounded-lg bg-secondary">
  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-lg border border-border/60 bg-card transition hover:-translate-y-0.5 hover:shadow-[var(--shadow-md)]">
      <Link to="/post/$slug" params={{ slug: post.slug }} className="relative block aspect-[16/10] overflow-hidden bg-secondary">
        {post.thumb && <img src={post.thumb} alt="" loading="lazy" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />}
